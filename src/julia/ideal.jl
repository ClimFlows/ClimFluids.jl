...
@fastmath @muladd @inlineall begin
    ...
    exner_functions(         gas::IPGT, (p,theta)::PCons) = zero(p), theta, gas.Cp*IPG_exner(gas, p)
    #   potential enthalpy
    ...
    exner_functions(         gas::IPGH, (p,hpot)::PCons) = zero(p), hpot, IPG_exner(gas, p)
    #   entropy
